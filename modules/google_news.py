...
class GoogleNewsModule:
    ...
    def search_news(
        ...
        Returns:
            ...
            - snippet (resumen)
        """
        lang = language or self.LANGUAGES.get(country, "en")
    ...
    def search_news_multilang(
        ...
        """
        Busca noticias en idioma local + inglés
        
        Args:
            query: Término de búsqueda
            country: Código de país
            include_english: Si True, busca también en inglés
            filter_relevance: Si debe filtrar noticias irrelevantes
        
        Returns:
            dict con noticias combinadas de ambos idiomas
        """
        from utils.countries import get_search_languages, get_language_flag
        
        # Obtener idiomas a buscar
        languages = get_search_languages(country)
        
        if not include_english:
            languages = languages[:1]
        
        all_news = []
        seen_links = set()  # Para deduplicar
            ...
            if result.get("success"):
                for news_item in result.get("news", []):
                    ...
                    if link and link not in seen_links:
                        seen_links.add(link)
                        # Añadir indicador de idioma
                        news_item["search_language"] = lang
                        news_item["language_flag"] = get_language_flag(lang)
