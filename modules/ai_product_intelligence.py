...
class AIProductAnalyzer:
    ...
    def _build_analysis_prompt(
        ...
        # Importar instrucción de idioma centralizada
        from utils.countries import get_ai_language_instruction
        language_instruction = get_ai_language_instruction(geo)
        ...
        for p in products:
            ...
            if p_name == product_name:
                ...
                if hasattr(p, 'category'):
                    cat = getattr(p, 'category', 'nicho')
                elif isinstance(p, dict):
                    cat = p.get('category', 'nicho')
                else:
                    cat = 'nicho'
                
                if hasattr(cat, 'value'):
                    cat = cat.value
                category = cat or 'nicho'
                break
    ...
    if analysis:
        ...
        with tabs[0]:
            try:
                ...
                st.error(f"Error mostrando clusters: {str(e)[:100]}")
        
        with tabs[1]:
            try:
                render_risk_opportunity_matrix(analysis.risk_matrix, products or [])
            except Exception as e:
                st.error(f"Error mostrando matriz de riesgo: {str(e)[:100]}")
        
        with tabs[2]:
            try:
                render_market_gaps(analysis.market_gaps)
            except Exception as e:
                st.error(f"Error mostrando gaps de mercado: {str(e)[:100]}")
        
        with tabs[3]:
            try:
                render_lifecycle_predictions(analysis.lifecycle_predictions, products or [])
            except Exception as e:
                st.error(f"Error mostrando predicciones: {str(e)[:100]}")
        
        with tabs[4]:
            try:
                render_ai_insights(analysis)
            except Exception as e:
                st.error(f"Error mostrando insights: {str(e)[:100]}")
