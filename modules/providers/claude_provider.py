"""
Claude Provider
Integración con Anthropic Claude API (2024-2025)
"""

from typing import Optional, Tuple
import json

# API Logger para tracking de costes
try:
    from modules.api_usage import log_ai_call
except ImportError:
...
class ClaudeProvider:
    ...
    MODEL = "claude-sonnet-4-5-20250929"  # Claude Sonnet 4.5 - Best coding & agentic
    MODEL_OPUS = "claude-opus-4-5-20251101"  # Claude Opus 4.5 - Most powerful
    MODEL_HAIKU = "claude-haiku-4-5-20251001"  # Claude Haiku 4.5 - Fast & cheap
    ...
    def _extract_content(self, response) -> str:
        """
        Extrae el contenido de la respuesta de forma segura.
        
        Args:
            response: Respuesta de la API de Claude
            
        Returns:
            Contenido de texto o cadena vacía
            
        Raises:
            ValueError: Si la respuesta está vacía
        """
        if not response.content:
            raise ValueError("Respuesta vacía de Claude API")
        
        content = response.content[0].text if response.content else ""
        
        if not content:
            raise ValueError("Sin contenido en la respuesta")
    ...
    def test_connection(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if response.content:
                return True, f"Conectado (Sonnet 4.5)"
            ...
            if "invalid_api_key" in error_msg.lower() or "401" in error_msg or "authentication" in error_msg.lower():
                ...
                return False, f"Modelo no disponible"
