...
def _get_fallback_emails() -> List[str]:
    ...
    try:
        ...
        emails = st.secrets.get("AUTH", {}).get("FALLBACK_EMAILS", [])
        if emails:
            return list(emails)
    except Exception:
        pass
    ...
    logger.warning("No hay FALLBACK_EMAILS configurados en secrets. Usar solo Supabase.")
    return []
...
def render_search_history() -> Optional[str]:
    ...
    for item in unique_history:
        ...
        with col1:
            if st.button(
                f"{score_icon} {keyword[:20]}{'...' if len(keyword) > 20 else ''}",
