...
def _get_language_instruction(geo: str) -> str:
    """
    Obtiene instrucción de idioma basada en código o nombre de país
    
    Args:
        geo: Código (ES) o nombre de país (España)
        
    Returns:
        Instrucción de idioma para el prompt
    """
    geo_lower = geo.lower().strip() if geo else ""
    
    # Mapeo de nombres/códigos a instrucciones
    LANGUAGE_MAP = {
        # Códigos
        ...
        "portugal": "Responde em português.",
        "francia": "Réponds en français.",
        "italia": "Rispondi in italiano.",
        "alemania": "Antworte auf Deutsch.",
        # Nombres en inglés
        "spain": "Responde en español.",
        "france": "Réponds en français.",
        "italy": "Rispondi in italiano.",
        "germany": "Antworte auf Deutsch.",
    }
    
    return LANGUAGE_MAP.get(geo_lower, "Respond in English.")
...
class MarketIntelligence:
    ...
    - sonar: Lightweight, rápido, económico (€0.92/M tokens)
    - sonar-pro: Búsqueda avanzada, 2x más citas (€2.76/M input, €13.80/M output)
    ...
    def _call_perplexity(
        ...
        try:
            ...
            # Payload simplificado - Perplexity busca web por defecto
            payload = {
            ...
            if not choices:
                self._last_error = "Perplexity no devolvió respuesta"
                return {"success": False, "error": self._last_error}
            
            content = choices[0].get("message", {}).get("content", "")
            if not content:
                self._last_error = "Respuesta vacía de Perplexity"
                return {"success": False, "error": self._last_error}
...
Responde SOLO con JSON válido."""
    ...
    def analyze_market(
        ...
        # Obtener instrucción de idioma
        language_instruction = _get_language_instruction(geo)
...
{language_instruction} Responde SOLO con JSON válido."""
