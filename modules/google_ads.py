...
class GoogleAdsKeywordPlanner:
    ...
    def get_keyword_volumes_multilang(
        self,
        keywords: List[str],
        geo: str = "ES",
        include_english: bool = True
    ) -> Dict[str, 'MultiLangKeywordMetrics']:
        """
        Obtiene volúmenes de búsqueda en idioma local + inglés
        ...
        Returns:
            Dict con keyword -> MultiLangKeywordMetrics (volumen local + inglés + total)
        """
        ...
        for lang in languages:
            results_by_lang[lang] = self.get_keyword_volumes(
                keywords=keywords,
                geo=geo,
                language=lang
            )
        ...
        for keyword in keywords:
            local_lang = languages[0]
            local_metrics = results_by_lang.get(local_lang, {}).get(keyword)
            en_metrics = results_by_lang.get("en", {}).get(keyword) if "en" in results_by_lang else None
            
            # Calcular total
            local_vol = local_metrics.avg_monthly_searches if local_metrics else 0
            en_vol = en_metrics.avg_monthly_searches if en_metrics else 0
            total_vol = local_vol + en_vol
            
            combined[keyword] = {
                "keyword": keyword,
                "local_language": local_lang,
                "local_volume": local_vol,
                "english_volume": en_vol,
                "total_volume": total_vol,
                "local_metrics": local_metrics,
                "english_metrics": en_metrics,
                # Usar métricas del idioma con mayor volumen para competencia/CPC
                "competition": (local_metrics or en_metrics).competition if (local_metrics or en_metrics) else "UNKNOWN",
                "avg_cpc": max(
                    (local_metrics.avg_cpc if local_metrics else 0),
                    (en_metrics.avg_cpc if en_metrics else 0)
                )
            }
...
# Importar funciones de formateo centralizadas
from utils.formatting import format_volume, format_change
