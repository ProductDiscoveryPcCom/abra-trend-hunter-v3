...
- Tendencias históricas (últimos 12-24 meses)
...
### ⚠️ IMPORTANTE: Autenticación y Scopes

Google Ads API usa **OAuth 2.0** para autenticación. Los scopes necesarios son:

```
https://www.googleapis.com/auth/adwords
```

Este es el **ÚNICO scope necesario** para acceder al Keyword Planner.
...
```
1. En Google Ads, ir a: Herramientas > API Center
2. Solicitar token de desarrollo (Basic Access es suficiente)
3. Esperar aprobación (1-2 días laborables)
4. El token tiene formato: AbCdEfGhIjKlMnOpQrStUvWx
```
...
3. Habilitar "Google Ads API" en APIs & Services > Library
4. Crear credenciales OAuth 2.0:
   - APIs & Services > Credentials > Create Credentials > OAuth client ID
   - Application type: Desktop app
   - Descargar el JSON de credenciales
5. Obtener refresh_token:
   - Usar el script de autenticación de Google Ads
   - O usar OAuth Playground: https://developers.google.com/oauthplayground
```

#### 4. Obtener Refresh Token (método OAuth Playground)
```
...
4. Introducir tu Client ID y Client Secret
5. En Step 1, añadir scope: https://www.googleapis.com/auth/adwords
6. Autorizar y obtener el refresh_token en Step 2
```
...
**YouTube Data API v3 usa API Key simple** (no OAuth) para acceso de solo lectura a datos públicos.

### ⚠️ NO necesita scopes

Para lectura de datos públicos, YouTube solo necesita una API Key. **No hay scopes.**

Si necesitas acceso a datos privados (tu canal, analytics), entonces sí necesitarías OAuth con estos scopes:
- `https://www.googleapis.com/auth/youtube.readonly` - Lectura de tu cuenta
- `https://www.googleapis.com/auth/youtube` - Acceso completo
- `https://www.googleapis.com/auth/youtube.force-ssl` - Similar a .readonly

### Configuración actual en Abra Trend Hunter

Usamos **solo API Key** porque accedemos a datos públicos:
...
| Operación | Coste en unidades | Límite diario |
|-----------|-------------------|---------------|
| search.list | 100 unidades | 10,000/día |
| videos.list | 1 unidad | 10,000/día |
| channels.list | 1 unidad | 10,000/día |

**Estimación de uso por búsqueda de marca:**
- 1 search.list = 100 unidades
- 5 videos.list (stats) = 5 unidades
- **Total: ~105 unidades por marca**
- **Máximo: ~95 marcas/día**
...
3. APIs & Services > Library > Buscar "YouTube Data API v3"
4. Click en "Enable"
5. APIs & Services > Credentials > Create Credentials > API Key
6. (Recomendado) Restringir la key solo a YouTube Data API v3
```

---

## 8. Resumen de APIs y Autenticación

| API | Tipo Auth | Scope/Key | Uso en Abra |
|-----|-----------|-----------|-------------|
| **SerpAPI** | API Key | Solo key | Google Trends, Related, News |
| **YouTube** | API Key | Solo key | Videos, estadísticas |
| **Google Ads** | OAuth 2.0 | `adwords` | Volúmenes reales (opcional) |
| **Perplexity** | API Key | Solo key | Market Intelligence |
| **OpenAI** | API Key | Solo key | AI Analysis (alternativa) |
| **Anthropic** | API Key | Solo key | AI Analysis (alternativa) |
