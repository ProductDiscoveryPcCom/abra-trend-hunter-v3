...
def render_product_section(
    ...
    if not analysis_result or not isinstance(analysis_result, dict):
        st.info("No hay datos de análisis de productos disponibles")
        return
    
    products = analysis_result.get("products") or []
    classified = analysis_result.get("classified") or {}
    thresholds = analysis_result.get("thresholds") or {}
    insights = analysis_result.get("insights") or {}
    shopping_products = analysis_result.get("shopping_products") or []
    ...
    # Análisis con IA (si hay suficientes productos)
    if len(products) >= 3:
        try:
...
def _render_shopping_card(product: Dict) -> None:
    ...
    with st.container():
        ...
        # Título (truncado con tooltip para ver completo)
        title = product.get("title", "Sin título")
        title_safe = html.escape(title)
        if len(title) > 50:
            title_short = html.escape(title[:50]) + "..."
            st.markdown(f'<span title="{title_safe}" style="cursor: help;">**{title_short}**</span>', unsafe_allow_html=True)
        else:
...
def render_opportunity_matrix(
    ...
    IMPORTANTE: Usa los mismos umbrales que la clasificación para que
    la posición visual coincida con la categoría asignada.
    
    Args:
        products: Lista de productos
        classified: Dict con productos clasificados
        brand: Nombre de la marca
        thresholds: Dict con umbrales {"volume": X, "growth": Y} de la clasificación
    """
    ...
    for product in products:
        ...
        if hasattr(product, 'name'):
            ...
            category = category.value if hasattr(category, 'value') else str(category) if category else 'nicho'
        ...
        # IMPORTANTE: Limitar growth a valores razonables para visualización
        # Los valores extremos (>300% o <-80%) distorsionan la matriz
        growth = max(-80, min(300, growth))
    ...
    if not x_values or all(v == 0 for v in x_values):
        st.warning(
            "No hay datos de volumen suficientes para la matriz. "
            "Esto puede ocurrir si:\n"
            "- La marca tiene poco volumen de búsqueda\n"
            "- Los datos de tendencias aún no se han cargado\n"
            "- Las APIs están tardando en responder"
        )
        
        # Mostrar lista de productos detectados aunque no tengamos volúmenes
        if names:
            st.info(f"Productos detectados: {', '.join(names[:10])}")
        return
    ...
    # CRÍTICO: Usar los mismos umbrales que la clasificación
    # Esto garantiza que la posición visual coincide con la categoría
    if thresholds:
        ...
        mid_x = (x_max + x_min) / 2
        mid_y = 0  # Crecimiento 0 como línea divisoria

    # Extender rangos si los umbrales están fuera
    ...
    # Líneas de umbral (las mismas que se usaron para clasificar)
    fig.add_hline(
    ...
    # Calcular tamaños proporcionales al volumen
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title="Volumen de búsqueda (índice Google Trends)",
            showgrid=True,
    ...
    if thresholds:
        st.caption(
            f"📊 Umbrales de clasificación: Volumen ≥ {thresholds.get('volume', 0):.0f} | "
            f"Crecimiento ≥ {thresholds.get('growth', 0):.1f}%"
        )
    
    # Disclaimer sobre limitaciones
    st.caption(
        "⚠️ **Nota**: El volumen es un índice relativo (0-100), no búsquedas reales. "
        "El crecimiento se limita a ±300% para legibilidad. Datos basados en Google Trends."
    )
...
def render_lifecycle_chart(products: List) -> None:
    ...
    # Disclaimer sobre limitaciones
    st.info(
        "⚠️ **Limitaciones**: Esta clasificación es una *estimación* basada en datos de búsqueda "
        "(Google Trends), no en datos reales de ventas o mercado. Los productos con datos limitados "
        "pueden clasificarse incorrectamente. Úsalo como orientación, no como dato definitivo."
    )
