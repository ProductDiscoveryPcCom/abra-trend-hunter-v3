...
def render_opportunity_matrix(
    ...
    fig.add_shape(type="rect", x0=x_min - x_padding, y0=mid_y, x1=mid_x, y1=y_max + y_padding,
                  fillcolor="rgba(139, 92, 246, 0.08)", line_width=0)  # Emergente
    fig.add_shape(type="rect", x0=mid_x, y0=mid_y, x1=x_max + x_padding, y1=y_max + y_padding,
                  fillcolor="rgba(245, 158, 11, 0.08)", line_width=0)  # Estrella
    fig.add_shape(type="rect", x0=mid_x, y0=y_min - y_padding, x1=x_max + x_padding, y1=mid_y,
                  fillcolor="rgba(59, 130, 246, 0.08)", line_width=0)  # Consolidado
    fig.add_shape(type="rect", x0=x_min - x_padding, y0=y_min - y_padding, x1=mid_x, y1=mid_y,
                  fillcolor="rgba(107, 114, 128, 0.08)", line_width=0)  # Nicho
    ...
    fig.add_annotation(x=x_min + x_range * 0.1, y=label_y_top, text="🚀 EMERGENTE", 
                       showarrow=False, font=dict(size=11, color="#8B5CF6"))
    fig.add_annotation(x=x_max - x_range * 0.1, y=label_y_top, text="⭐ ESTRELLA", 
                       showarrow=False, font=dict(size=11, color="#F59E0B"))
    fig.add_annotation(x=x_max - x_range * 0.1, y=label_y_bottom, text="💎 CONSOLIDADO", 
                       showarrow=False, font=dict(size=11, color="#3B82F6"))
    fig.add_annotation(x=x_min + x_range * 0.1, y=label_y_bottom, text="👀 NICHO", 
    ...
    for i, product in enumerate(sorted_products[:10]):
        ...
        with col4:
            ...
            if values and len(values) >= 3:
                ...
                st.plotly_chart(fig, width="stretch", key=f"spark_{name}_{i}")
