...
def _render_product_grid(products: list) -> None:
    ...
    # Filtrar y ordenar
    filtered = [p for p in products if p is not None and hasattr(p, 'price') and min_price <= (p.price or 0) <= max_price]

    if sort_by == "orders":
        filtered.sort(key=lambda x: getattr(x, 'orders', 0) or 0, reverse=True)
    elif sort_by == "price_asc":
        filtered.sort(key=lambda x: getattr(x, 'price', 0) or 0)
    elif sort_by == "price_desc":
        filtered.sort(key=lambda x: getattr(x, 'price', 0) or 0, reverse=True)
    elif sort_by == "rating":
        filtered.sort(key=lambda x: getattr(x, 'rating', 0) or 0, reverse=True)
...
def _render_price_analysis(products: list, metrics: any) -> None:
    ...
    with col1:
        ...
        prices = [p.price for p in products if p is not None and hasattr(p, 'price') and p.price and p.price > 0 and p.price < 500]
    ...
    df_scatter = pd.DataFrame([
        {
            "Precio": getattr(p, 'price', 0),
            "Pedidos": getattr(p, 'orders', 0),
            "Rating": getattr(p, 'rating', 0),
            "Producto": (getattr(p, 'title', '') or '')[:30]
        }
        for p in products if p is not None and hasattr(p, 'price') and p.price and p.price > 0 and p.price < 500
    ])
...
def _render_shop_analysis(products: list) -> None:
    ...
    for p in products:
        if p is None:
            continue
        shop = getattr(p, 'shop_name', None) or "Unknown"
        shops[shop]["products"] += 1
        shops[shop]["total_orders"] += getattr(p, 'orders', 0) or 0
        price = getattr(p, 'price', 0) or 0
        if price > 0:
            shops[shop]["avg_price"].append(price)
        rating = getattr(p, 'rating', 0) or 0
        if rating > 0:
            shops[shop]["ratings"].append(rating)
