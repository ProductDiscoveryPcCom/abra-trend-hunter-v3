...
YOUTUBE_LANGUAGES = {
    "all": {"name": "Todos los idiomas", "flag": "🌍"},
...
def render_language_selector(key: str = "yt_lang") -> str:
    """
    Renderiza selector de idioma para filtrar videos.
    
    Args:
        key: Key única para el widget
        
    Returns:
        Código de idioma seleccionado
    """
    options = list(YOUTUBE_LANGUAGES.keys())
    labels = [f"{info['flag']} {info['name']}" for info in YOUTUBE_LANGUAGES.values()]
    
    selected = st.selectbox(
        "🌍 Filtrar por idioma",
        options=options,
        format_func=lambda x: f"{YOUTUBE_LANGUAGES[x]['flag']} {YOUTUBE_LANGUAGES[x]['name']}",
        key=key,
        help="Filtra los videos por idioma del contenido"
    )
...
def render_youtube_deep_dive(
    ...
    col_header, col_lang = st.columns([3, 1])
    ...
    # Filtrar videos por idioma si no es "all"
    filtered_deep_dive = deep_dive
    if selected_lang != "all" and hasattr(deep_dive, 'videos_by_type') and deep_dive.videos_by_type:
        # Nota: El filtrado real requeriría modificar deep_dive_analysis
        # Por ahora mostramos una estadística del idioma seleccionado
        if hasattr(deep_dive, 'language_stats') and deep_dive.language_stats:
            lang_videos = [ls for ls in deep_dive.language_stats if ls.language == selected_lang]
            if lang_videos:
                st.info(f"📊 Videos en {YOUTUBE_LANGUAGES.get(selected_lang, {}).get('name', selected_lang)}: **{lang_videos[0].video_count}** ({lang_videos[0].percentage:.0f}% del total)")
