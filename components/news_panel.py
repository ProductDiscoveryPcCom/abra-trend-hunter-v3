...
def _render_news_card(item: dict) -> None:
    ...
    lang_flag = item.get("language_flag", "")  # Indicador de idioma
    ...
    with st.container():
        ...
        # Añadir bandera de idioma si está disponible
        lang_indicator = f" {lang_flag}" if lang_flag else ""
        st.caption(f"📰 {source} · {date}{lang_indicator}")
...
def render_news_comparison(
    ...
    # Protección contra división por cero si todos los counts son 0
    if max_count == 0:
        max_count = 1
