...
def render_keyword_table(
    ...
    if not categorized_data or not isinstance(categorized_data, dict):
        st.info("No hay datos de búsquedas relacionadas")
        return

    categorized = categorized_data.get("categorized") or {}
    total = categorized_data.get("total_count", 0)

    all_items = categorized.get("all") or []
    questions = categorized.get("questions") or []
    comparatives = categorized.get("comparatives") or []
    others = categorized.get("others") or []
