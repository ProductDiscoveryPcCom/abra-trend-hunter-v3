...
def render_animated_loading(
    title: str = "Analizando datos",
    steps: List[str] = None,
    current_step: int = 0
):
    """
    Renderiza un loading animado con pasos.
    ...
    """
    default_steps = ["Conectando APIs", "Obteniendo datos", "Procesando", "Finalizando"]
    steps = steps or default_steps
    
    st.markdown(f"""
    <div style="text-align: center; padding: 40px; background: linear-gradient(135deg, #F9FAFB 0%, #F3F4F6 100%);
         border-radius: 16px; border: 1px solid #E5E7EB;">
        <div style="font-size: 3rem; margin-bottom: 16px;">🔮</div>
        <h3 style="margin: 0 0 8px; color: #1A1A2E;">{html.escape(title)}</h3>
...
def render_metric_card(
    ...
    if delta:
        if delta_color == "green" or (delta_color is None and delta.startswith("+")):
            d_color = "#10B981"
            d_bg = "#D1FAE5"
        elif delta_color == "red" or (delta_color is None and delta.startswith("-")):
            d_color = "#EF4444"
            d_bg = "#FEE2E2"
        else:
            d_color = "#6B7280"
        ...
        delta_html = f'''
        <span style="background: {d_bg}; color: {d_color}; padding: 2px 8px; 
             border-radius: 6px; font-size: 0.75rem; font-weight: 600; margin-left: 8px;">
            {html.escape(delta)}
        </span>
        '''
    else:
    ...
    <div style="background: white; padding: 20px; border-radius: 12px;
        ...
        <div style="display: flex; align-items: center; margin-bottom: 8px;">
            {icon_html}
            <span style="color: #6B7280; font-size: 0.85rem; font-weight: 500;">{html.escape(label)}</span>
        </div>
        <div style="display: flex; align-items: baseline;">
            <span style="font-size: 1.75rem; font-weight: 700; color: #1A1A2E;">{html.escape(value)}</span>
            {delta_html}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_alert(
    ...
    """
    Renderiza una alerta estilizada.
    ...
    Args:
        ...
        key: Key para botón de cerrar
    
    Returns:
        True si se cerró, False si no
    """
    styles = {
    ...
    <div style="background: {style['bg']}; border-left: 4px solid {style['border']};
         padding: 16px; border-radius: 8px; display: flex; align-items: flex-start; gap: 12px;">
        <span style="font-size: 1.25rem;">{display_icon}</span>
        <div style="flex: 1; color: {style['text']}; font-size: 0.9rem; line-height: 1.5;">
            {html.escape(message)}
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if dismissible and key:
        return st.button("✕ Cerrar", key=key)
...
def render_stat_row(stats: List[dict]):
    ...
    Args:
        stats: Lista de dicts con keys: label, value, icon (opcional), color (opcional)
    """
    ...
    for col, stat in zip(cols, stats):
        with col:
            ...
            color = stat.get("color", "#7C3AED")
            
            st.markdown(f"""
            <div style="text-align: center; padding: 16px; background: {color}08;
                 border-radius: 12px; border: 1px solid {color}20;">
                <div style="font-size: 1.5rem; margin-bottom: 4px;">{icon}</div>
                <div style="font-size: 1.25rem; font-weight: 700; color: {color};">{html.escape(str(value))}</div>
                <div style="font-size: 0.75rem; color: #6B7280;">{html.escape(str(label))}</div>
            </div>
            """, unsafe_allow_html=True)


def render_section_divider(title: str = None, icon: str = None):
    """
    Renderiza un divisor de sección.
    ...
    if title:
        ...
        <div style="display: flex; align-items: center; margin: 24px 0 16px;">
            ...
            <span style="padding: 0 16px; color: #6B7280; font-size: 0.85rem; font-weight: 500;">
        ...
        st.markdown("<hr style='margin: 24px 0; border: none; border-top: 1px solid #E5E7EB;'>", 
                   unsafe_allow_html=True)
