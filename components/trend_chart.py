...
# Importar función de formateo centralizada
from utils.formatting import format_volume


def calculate_multi_period_growth(values: List[float], dates: List[datetime]) -> Dict[str, float]:
    """
    Calcula el crecimiento para múltiples períodos de tiempo.
    
    Args:
        values: Lista de valores del índice
        dates: Lista de fechas correspondientes
    
    Returns:
        Dict con crecimientos para 1m, 3m, 6m, 12m, 5y
    """
    if len(values) < 2 or len(dates) < 2:
        return {"1m": 0, "3m": 0, "6m": 0, "12m": 0, "5y": 0}
    ...
    for period_name, delta in periods.items():
        ...
        # Encontrar el valor más cercano a esa fecha
        ...
        if len(past_df) > 0:
            ...
            # Si no hay datos suficientes, usar el primero disponible
            if len(df) > 1:
        ...
        results[period_name] = round(growth, 1)
    
    return results


def render_growth_metrics(growths: Dict[str, float]) -> None:
    """
    Renderiza las métricas de crecimiento multi-período.
    
    Args:
        growths: Dict con crecimientos para cada período
    """
    cols = st.columns(5)
    ...
    for i, (period, label) in enumerate(period_labels.items()):
        growth = growths.get(period, 0)
        ...
        with cols[i]:
            ...
            st.markdown(f"""
            <div style="text-align: center; padding: 8px; background: {color}15; border-radius: 8px;">
...
def render_trend_chart(
    ...
    # Crecimientos multi-período
    st.markdown("##### 📊 Crecimiento por período")
    growths = calculate_multi_period_growth(
        values=df[y_col].tolist(),
        dates=df["date"].tolist()
    )
    render_growth_metrics(growths)
...
def render_trend_with_events(
    ...
    """
    Renderiza gráfico de tendencia con eventos/noticias marcados
    
    Args:
        timeline_data: Datos de timeline de Google Trends
        news_data: Lista de noticias con 'date' y 'title'
        keyword: Keyword para el título
        height: Altura del gráfico
    """
    if not timeline_data:
        st.warning("No hay datos de timeline")
        return
    ...
    for point in timeline_data:
        ...
        if date_str and point_values:
            try:
                date = _parse_date_flexible(date_str)
                if date:
                    val = point_values[0].get("extracted_value", 0)
                    dates.append(date)
                    values.append(float(val) if val else 0)
    ...
    fig.add_trace(go.Scatter(
        ...
        fill='tozeroy',
        fillcolor='rgba(124, 58, 237, 0.1)',
        hovertemplate='%{x|%b %Y}<br>Índice: %{y:.0f}<extra></extra>'
    ))
    
    # Añadir marcadores de eventos/noticias
    if news_data:
        event_dates = []
        event_values = []
        event_texts = []
        ...
        for news in news_data[:5]:
            ...
            news_title = news.get("title", "")[:40]
            
            if news_date_str:
                try:
                    news_date = _parse_date_flexible(news_date_str)
                    if news_date and dates:
                        closest_idx = min(
                            range(len(dates)),
                            key=lambda i: abs((dates[i] - news_date).days)
                        )
                        event_dates.append(news_date)
                        event_values.append(values[closest_idx])
                        event_texts.append(news_title)
        ...
        if event_dates:
            fig.add_trace(go.Scatter(
                ...
                hovertemplate='<b>%{customdata}</b><br>%{x|%b %Y}<extra></extra>',
                customdata=event_texts
            ))
    
    fig.update_layout(
        title=dict(
            text=f"📈 Tendencia: {keyword}" if keyword else "📈 Tendencia con Eventos",
            font=dict(size=16)
        ),
...
def _parse_date_flexible(date_str: str) -> Optional[datetime]:
    ...
    if len(parts) >= 2:
        ...
        if month_str in month_map:
            try:
                year = int(parts[-1])
                return datetime(year, month_map[month_str], 15)
            except ValueError:
                pass
    return None
