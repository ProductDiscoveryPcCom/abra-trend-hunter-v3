...
def render_trend_chart(
    ...
    # Fecha actual como referencia para fallbacks
    now = datetime.now()
    ...
    for i, point in enumerate(timeline_data):
        ...
        if not date_str:
            # Sin fecha, calcular hacia atrás desde hoy
            weeks_back = len(timeline_data) - i - 1
            date = now - pd.Timedelta(weeks=weeks_back)
            dates.append(date)
            values.append(val)
            continue
        ...
        # Último fallback: calcular hacia atrás desde hoy
        if date is None:
            weeks_back = len(timeline_data) - i - 1
            date = now - pd.Timedelta(weeks=weeks_back)
