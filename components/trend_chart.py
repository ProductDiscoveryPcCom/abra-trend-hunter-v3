...
def render_trend_chart(
    ...
    dates = []
    values = []
    raw_dates = []  # Para debug
    
    for i, point in enumerate(timeline_data):
        # Extraer valor primero (más importante)
        val = 0
        ...
        raw_dates.append(date_str)  # Debug
        
        if not date_str:
            # Sin fecha, usar índice
            date = datetime(2024, 1, 1) + pd.Timedelta(weeks=i)
            dates.append(date)
            values.append(val)
            continue
        ...
        # Intentar múltiples formatos (inglés y español)
        formats_to_try = [
            "%b %d, %Y",      # Nov 3, 2024
        ...
        if date is None:
            ...
            # Buscar patrón "Mes Año" o "Año-Mes"
        ...
        # Último fallback: usar índice con fecha base
        if date is None:
            date = datetime(2024, 1, 1) + pd.Timedelta(weeks=i)
    ...
    if not values or all(v == 0 for v in values):
        ...
        # Debug info
        if raw_dates:
            st.caption(f"Formatos de fecha recibidos: {raw_dates[:3]}...")
        return
    ...
    if show_volume_estimate and api_key:
        try:
            ...
            # Si falla, seguir con índice
            st.caption(f"⚠️ Usando índice de tendencia (0-100)")
            volume_data = None
    
    # Decidir qué columna usar para el gráfico
    ...
    hover_texts = [f"<b>{d.strftime('%b %Y')}</b><br>{hover_label}: {format_volume(v)}" 
    ...
    fig.add_hline(
        ...
        line_dash="dash",
        line_color="#94A3B8",
        annotation_text=f"Media: {format_volume(avg_value)}",
    ...
    with col1:
        if volume_data:
            st.metric(
                label="Búsquedas/mes",
                value=format_volume(current_value),
                help="Volumen mensual estimado"
            )
        else:
            st.metric(
                label="Índice actual",
    ...
    with col4:
        ...
        st.metric(
            label="Pico máximo",
            value=format_volume(peak)
        )
    
    # Indicador de estimación
    if volume_data:
        confidence = volume_data["current"].get("confidence_label", "Media")
        st.caption(f"📊 Volumen estimado (confianza: {confidence}) · Basado en Google Trends + análisis de resultados")
