...
# Importar funciones centralizadas de formateo
...
def render_related_queries(
    ...
    # Validar que queries sea un diccionario
    if not queries or not isinstance(queries, dict):
        st.warning("⚠️ No se pudieron obtener búsquedas relacionadas. Esto puede ocurrir si la API tardó demasiado o si el término tiene poco volumen de búsqueda.")
        return
    
    # Verificar si hay datos
    rising = queries.get("rising") or []
    top = queries.get("top") or []
    if not rising and not top:
        st.info("No hay búsquedas relacionadas para este término en la región seleccionada.")
        return
    ...
    with tab1:
        rising_queries = queries.get("rising") or []
        _render_query_list(rising_queries, is_rising=True, has_real_volumes=has_real_volumes)

    with tab2:
        top_queries = queries.get("top") or []
        _render_query_list(top_queries, is_rising=False, has_real_volumes=has_real_volumes)
...
def render_related_topics(
    ...
    # Validar que topics sea un diccionario
    if not topics or not isinstance(topics, dict):
        st.info("No hay datos de topics disponibles")
        return

    tab1, tab2 = st.tabs(["📈 Rising", "🏆 Top"])

    with tab1:
        rising_topics = topics.get("rising") or []
        _render_topic_list(rising_topics, is_rising=True)

    with tab2:
        top_topics = topics.get("top") or []
        _render_topic_list(top_topics, is_rising=False)
