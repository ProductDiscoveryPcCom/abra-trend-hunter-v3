...
def _render_score_ring(
    ...
    # Explicaciones por rango
    SCORE_TIPS = {
        "Trend Score": {
            (0, 30): "❄️ Baja actividad actual",
            (31, 50): "🌤️ Interés estable, sin crecimiento notable",
            (51, 70): "🔥 Crecimiento activo, buen momento",
            (71, 100): "🚀 Tendencia muy fuerte, alta demanda"
        },
        "Potential Score": {
            (0, 30): "📉 Mercado maduro o en declive",
            (31, 50): "📊 Posible crecimiento gradual",
            (51, 70): "📈 Señales de crecimiento próximo",
            (71, 100): "💎 Alta probabilidad de explosión"
        }
    }
    
    # Obtener tip según score
    score_tip = ""
    if title in SCORE_TIPS:
        for (min_s, max_s), tip in SCORE_TIPS[title].items():
    ...
    st.plotly_chart(fig, use_container_width=True, key=f"ring_{title}")

    # Mostrar tip basado en score
    if score_tip:
