...
def render_logo():
    ...
    mascot_html = '<span style="font-size: 2.5rem;">🔮</span>'  # Fallback
...
def render_api_status():
    ...
    apis_config = [
        ...
        },
        {
            ...
            "required": False,
            "test_module": None,  # No tiene test_connection aún
...
def render_loading_state(message: str = "Analizando tendencias..."):
    ...
    # Intentar cargar la imagen de la mascota
    mascot_html = '<div class="loading" style="font-size: 2.5rem;">🔮</div>'  # Fallback
    ...
    st.markdown(
        ...
        justify-content: center; padding: 40px; gap: 16px;">
            {mascot_html}
            <span style="color: #7C3AED; font-size: 1rem; font-weight: 500;">{safe_message}</span>
...
def render_empty_state():
    ...
    # Intentar cargar la imagen de la mascota
    mascot_html = '<div style="font-size: 5rem; margin-bottom: 16px;">🔮</div>'  # Fallback
    ...
    st.markdown(
        ...
        <div style="text-align: center; padding: 60px 20px; color: #64748B;">
            ...
            <div style="display: flex; gap: 8px; justify-content: center; flex-wrap: wrap;">
                <span style="background: linear-gradient(135deg, #F5C518 0%, #FCD34D 100%); 
                      padding: 8px 16px; border-radius: 20px; font-size: 0.85rem; 
                      color: #1A1A2E; font-weight: 500;">Ej: Beelink</span>
                <span style="background: linear-gradient(135deg, #7C3AED 0%, #A855F7 100%); 
                      padding: 8px 16px; border-radius: 20px; font-size: 0.85rem; 
                      color: white; font-weight: 500;">Framework Laptop</span>
                <span style="background: linear-gradient(135deg, #10B981 0%, #34D399 100%); 
                      padding: 8px 16px; border-radius: 20px; font-size: 0.85rem; 
                      color: white; font-weight: 500;">Steam Deck</span>
            </div>
        </div>
        ''',
        unsafe_allow_html=True
