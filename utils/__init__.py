...
# Import de funciones de formateo centralizadas
...
def render_logo():
    ...
    with col1:
        # Intentar múltiples rutas para la imagen
        possible_paths = [
        ...
        for mascot_path in possible_paths:
            if mascot_path.exists():
                try:
                    # Usar base64 para mayor compatibilidad
                    with open(mascot_path, "rb") as f:
                        img_base64 = base64.b64encode(f.read()).decode()
                    st.markdown(
                        f'<img src="data:image/png;base64,{img_base64}" width="56" style="border-radius: 8px;">',
        ...
        if not mascot_loaded:
            # Fallback: emoji con estilo
            st.markdown(
                '''<div style="width: 56px; height: 56px; 
                    background: linear-gradient(135deg, #F5C518, #7C3AED);
...
def check_api_keys() -> dict:
    ...
    """
    try:
        # Google Ads requiere todas sus credenciales
        ...
        return {
            ...
            "google_ads": google_ads_configured,
            "aliexpress": bool(st.secrets.get("ALIEXPRESS_KEY", "")),
            "amazon": bool(st.secrets.get("AMAZON_API_KEY", "")),
            "tiktok": bool(st.secrets.get("TIKTOK_API_KEY", "")),
            "google_shopping": bool(st.secrets.get("GOOGLE_SHOPPING_API_KEY", ""))
        }
    except Exception:
        return {
            "serpapi": False,
...
def render_api_status():
    ...
    apis_config = [
        ...
        },
        {
            "name": "Google Ads",
            "key": "google_ads",
            "configured": status["google_ads"],
            "required": False,
            "test_module": "google_ads",
            "secret_key": "GOOGLE_ADS_DEVELOPER_TOKEN"
        },
        {
            "name": "AliExpress",
            "key": "aliexpress",
            "configured": status["aliexpress"],
            "required": False,
            "test_module": None,
            "secret_key": "ALIEXPRESS_KEY"
        },
        {
            "name": "Amazon",
            "key": "amazon",
            "configured": status["amazon"],
            "required": False,
            "test_module": None,
            "secret_key": "AMAZON_API_KEY"
        },
        {
            "name": "TikTok",
            "key": "tiktok",
            ...
            "required": False,
            "test_module": None,
            "secret_key": "TIKTOK_API_KEY"
        },
        {
            "name": "Google Shopping",
            "key": "google_shopping",
            "configured": status["google_shopping"],
            "required": False,
            "test_module": None,
            "secret_key": "GOOGLE_SHOPPING_API_KEY"
        },
    ]
...
def _verify_all_connections(apis_config: list):
    ...
    for api in apis_config:
        ...
        try:
            ...
            elif test_module == "google_ads":
                from modules.google_ads import GoogleAdsKeywordPlanner
                module = GoogleAdsKeywordPlanner()
                if module.is_available:
                    st.session_state.api_connection_status[key] = (True, "Conectado")
                else:
                    error = module.last_error or "No disponible"
                    st.session_state.api_connection_status[key] = (False, error[:30])
            
            elif test_module is None:
                # API configurada pero sin test_connection implementado
...
# format_number ahora se importa desde utils.formatting (ver imports arriba)
...
# NOTA: render_search_history se usa desde modules.auth_email (versión mejorada con avatares)
