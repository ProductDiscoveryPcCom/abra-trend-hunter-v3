...
Previene XSS, inyección, SSRF y maneja valores nulos/cero
"""
...
# IPs y rangos bloqueados (privados, loopback, link-local, etc.)
BLOCKED_IP_RANGES = [
...
# Puertos peligrosos
BLOCKED_PORTS = [22, 23, 25, 53, 135, 139, 445, 3306, 5432, 6379, 27017]

# Dominios permitidos (whitelist para retailers conocidos)
ALLOWED_DOMAINS = [
    'pccomponentes.com',
    'amazon.es', 'amazon.com', 'amazon.de', 'amazon.fr', 'amazon.it', 'amazon.co.uk',
    'mediamarkt.es', 'mediamarkt.de', 'mediamarkt.fr', 'mediamarkt.it',
    'aliexpress.com',
    'newegg.com',
    'alternate.es', 'alternate.de',
    'coolmod.com',
    'ldlc.com',
    'topachat.com',
]
...
def is_ip_blocked(ip_str: str) -> bool:
    """
    Verifica si una IP está en rangos bloqueados
    
    Args:
        ip_str: String de IP a verificar
...
def validate_url_safe(url: str, allow_custom_domains: bool = False) -> Tuple[bool, str]:
    """
    Valida que una URL sea segura para hacer requests (previene SSRF)
    
    Args:
        url: URL a validar
        allow_custom_domains: Si permitir dominios fuera de la whitelist
    ...
    try:
        ...
        # 1. Verificar esquema (solo HTTP/HTTPS)
        ...
        for blocked in BLOCKED_DOMAINS:
            if hostname_lower == blocked or hostname_lower.endswith('.' + blocked):
                return False, f"Dominio bloqueado: {hostname}"
        
        # 4. Verificar si es una IP directa
        try:
            if is_ip_blocked(hostname):
                return False, f"IP en rango bloqueado: {hostname}"
        except ValueError:
            pass  # No es una IP, es un dominio
        
        # 5. Resolver DNS y verificar IP resultante
        try:
            ip = socket.gethostbyname(hostname)
            if is_ip_blocked(ip):
                return False, f"El dominio resuelve a IP bloqueada: {ip}"
        except socket.gaierror:
            return False, f"No se puede resolver el dominio: {hostname}"
        
        # 6. Verificar puerto
        port = parsed.port
        if port and port in BLOCKED_PORTS:
            return False, f"Puerto bloqueado: {port}"
        
        # 7. Verificar whitelist (si está habilitada)
        if not allow_custom_domains:
            ...
            if not domain_ok:
                return False, f"Dominio no en whitelist: {hostname}"
        
        return True, "OK"
        
    except Exception as e:
        return False, f"Error validando URL: {str(e)[:100]}"
...
def sanitize_url(url: str) -> str:
    ...
    url = ''.join(c for c in str(url).strip() if c.isprintable())
